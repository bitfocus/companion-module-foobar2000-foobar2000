	...
	try {
		...
		if (json1.player.activeItem.playlistId !== '') {
			...
			if (json.player.activeItem.index > 0) {
				index = 1
				nextTrackName =
					items.length >= 3 && items[2] && Array.isArray(items[2].columns) && typeof items[2].columns[0] === 'string'
						? items[2].columns[0]
						: ''
				previousTrackName =
					items.length >= 1 && items[0] && Array.isArray(items[0].columns) && typeof items[0].columns[0] === 'string'
						? items[0].columns[0]
						: ''
			} else {
				index = 0
				nextTrackName =
					items.length >= 2 && items[1] && Array.isArray(items[1].columns) && typeof items[1].columns[0] === 'string'
						? items[1].columns[0]
						: ''
				previousTrackName = ''
			}
			...
			const currentTrackName =
				items.length > index &&
				items[index] &&
				Array.isArray(items[index].columns) &&
				typeof items[index].columns[0] === 'string'
					? items[index].columns[0]
					: ''
